...
class SystemLogService:
    ...
    async def create_log(self, log_data: SystemLogCreate):
        ...
        try:
            new_log = {
                ...
                "employee_suffix": log_data.employee_suffix,  # ✅ Added
    ...
    async def get_user_activity(self, user_id: int):
        ...
        try:
            result = self.supabase.table('system_logs').select('*').eq('user_id', user_id).order('log_time', desc=True).execute()
    ...
    async def get_employee_history(self, employee_id: int):
        ...
        try:
            result = self.supabase.table('system_logs').select('*').eq('employee_id', employee_id).order('log_time', desc=True).execute()
