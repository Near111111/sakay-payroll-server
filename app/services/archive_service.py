...
class ArchiveService:
    ...
    async def get_all_archives(self):
        ...
        try:
            ...
            for archive in archives.data:
                ...
                count = self.supabase.table('archive_payrolls').select('archive_payroll_id', count='exact').eq('archive_report_id', archive['archive_report_id']).execute()
    ...
    async def get_archive_by_id(self, archive_report_id: int):
        ...
        try:
            # Get archive report
            archive = self.supabase.table('archive_reports').select('*').eq('archive_report_id', archive_report_id).execute()
            ...
            payrolls = self.supabase.table('archive_payrolls').select('*').eq('archive_report_id', archive_report_id).execute()
    ...
    async def delete_archive(self, archive_report_id: int):
        ...
        try:
            # Check if exists
            archive = self.supabase.table('archive_reports').select('archive_report_id').eq('archive_report_id', archive_report_id).execute()
