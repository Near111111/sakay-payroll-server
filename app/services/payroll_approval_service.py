...
class PayrollApprovalService:
    ...
    async def unapprove(self, period_start: str, period_end: str, approver_role: str, username: str):
        try:
            if approver_role not in ("accounting", "ceo"):
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="approver_role must be 'accounting' or 'ceo'")
            ...
            if not existing.data:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No approval record found for this period")

            record = existing.data[0]

            if approver_role == "accounting":
                if not record.get('approved_by_accounting'):
                    return {"message": "Not yet approved by accounting", "already_unapproved": True}
                result = db_execute(
                    """
                    UPDATE payroll_approvals
                    SET approved_by_accounting = FALSE, accounting_approved_at = NULL
                    WHERE approval_id = :approval_id RETURNING *
                    """,
                    {"approval_id": record['approval_id']}
                )
            else:
                if not record.get('approved_by_ceo'):
                    return {"message": "Not yet approved by CEO", "already_unapproved": True}
                result = db_execute(
                    """
                    UPDATE payroll_approvals
                    SET approved_by_ceo = FALSE, ceo_approved_at = NULL
                    WHERE approval_id = :approval_id RETURNING *
                    """,
                    {"approval_id": record['approval_id']}
                )
            ...
            if not result.data:
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to unapprove")

            return {
                "message": f"Unapproved by {approver_role}",
                "approver_role": approver_role,
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to unapprove: {str(e)}")